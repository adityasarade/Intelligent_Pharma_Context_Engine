...
def remove_shadows(image: np.ndarray) -> np.ndarray:
    """
    Removes shadows/glare using morphological operations (light MVP approach).
    For a more robust glare removal, we might need inpainting, but this helps with uneven lighting.
    """
...
def preprocess_pipeline(image_path: str, debug_output: bool = False) -> Dict[str, np.ndarray]:
    """
    Runs the full Stage 1 image preprocessing pipeline.
    Returns specific intermediate images for traceability.
    """
    ...
    # Return dictionary of stages for debugging/OCR
    return {
        "original": original,
        "normalized": normalized,
        "denoised": denoised,
        "gray": gray,
        "enhanced_gray": enhanced # This is the primary input for OCR
    }
